...
| Cadence | **Per calendar**: manual only / 5 / 10 / 15 / 30 / 60 min (default 5), plus an immediate push right after a booking or cancellation | A `BackgroundService` ticking once a minute; `SyncSchedule.IsDue` (pure, tested) decides per connection from its own `SyncEveryMinutes` and `LastSyncAt`, with 30 s of slack so "every 5 minutes" is not every 6. Set from the Schedule page's sync badge or the Calendars row. No Hangfire: this service has no queue, and this worker is exactly what a Hangfire recurring job would be. |
